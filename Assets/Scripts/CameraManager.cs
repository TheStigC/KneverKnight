...
public class CameraManager : MonoBehaviour {
    ...
    void Start() {
        ...
        orthographicsize_base = thisCamera.orthographicSize;
        ...
        if (listAreaNodes.Count == 0)
            ...
            */
        if (listAreaNodes.Count<=0 && boundariesContainer!=null)
        {
            foreach (Transform child in boundariesContainer.transform)
            {
                listAreaNodes.Add(child.gameObject);
    ...
    void Update() {
        ...
        if (listAreaNodes.Count > 0) {
            ...
            if (GetAreaRect(currentArea).width < (thisCamera.orthographicSize * thisCamera.aspect) * 2) {
                ...
                newPosition.x = Mathf.Clamp(followtarget.x,
                    listAreaNodes[currentArea].transform.position.x + (thisCamera.orthographicSize * thisCamera.aspect),
                    listAreaNodes[currentArea].transform.GetChild(0).position.x - (thisCamera.orthographicSize * thisCamera.aspect));
            }
            ...
            if (GetAreaRect(currentArea).height < thisCamera.orthographicSize * 2) {
                ...
                listAreaNodes[currentArea].transform.GetChild(0).position.y + thisCamera.orthographicSize,
                listAreaNodes[currentArea].transform.position.y - thisCamera.orthographicSize);
            }
    ...
    private IEnumerator I_ZoomIn(float _newSize) {
        while (thisCamera.orthographicSize > _newSize) {
            thisCamera.orthographicSize -= Mathf.Min(zoomspeed, thisCamera.orthographicSize - _newSize);
            yield return new WaitForEndOfFrame();
        }

        thisCamera.orthographicSize = _newSize;
        yield return null;
    }

    private IEnumerator I_ZoomOut(float _newSize) {
        while (thisCamera.orthographicSize < _newSize) {
            thisCamera.orthographicSize += Mathf.Min(zoomspeed, _newSize - thisCamera.orthographicSize);
            yield return new WaitForEndOfFrame();
        }
    ...
    public Rect GetCameraRect() {
        Rect rect = new Rect(transform.position.x - (thisCamera.orthographicSize * thisCamera.aspect), transform.position.y - thisCamera.orthographicSize,
            (thisCamera.orthographicSize * thisCamera.aspect) * 2, -thisCamera.orthographicSize * 2);
        return rect;
    }
    ...
    private void CameraControls() {
        ...
        if (Input.GetKeyDown(KeyCode.Z)) {
            FocusPosition = thisCamera.ScreenToWorldPoint(Input.mousePosition);
        }
